...
/**
 * Convert a date string to local timezone YYYY-MM-DD format.
 * TickTick API returns UTC dates (e.g., "2026-03-04T16:00:00.000+0000"),
 * but we need to display them in the user's local timezone.
 * @param {string} dateStr - ISO 8601 date string from TickTick API
 * @returns {string} YYYY-MM-DD in local timezone
 */
function formatLocalDate(dateStr) {
  if (!dateStr) return '';
  // Pure date strings (YYYY-MM-DD) without time info — return as-is
...
/**
 * Format a datetime string to local timezone readable format.
 * @param {string} dateStr - ISO 8601 datetime string
 * @returns {string} YYYY-MM-DD HH:mm in local timezone
 */
...
function formatArray(arr) {
  ...
  if ('title' in first) {
    ...
    for (const item of arr) {
      ...
      const due = formatLocalDate(item.dueDate).padEnd(10);
...
function formatTaskDetail(task) {
  ...
  if (task.dueDate) lines.push(`Due: ${formatLocalDate(task.dueDate)}`);
  ...
  if (task.createdTime) lines.push(`Created: ${formatLocalDateTime(task.createdTime)}`);
  if (task.modifiedTime) lines.push(`Modified: ${formatLocalDateTime(task.modifiedTime)}`);
  return lines.join('\n');
}
